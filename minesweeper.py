...
class MinesweeperAI:
    ...
    def add_knowledge(self, cell, count):
        ...
        current_cells, current_count = set(), count
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                current_cell = (i, j)
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    if current_cell in self.mines:
                        current_count -= 1
                    elif current_cell not in self.safes:
                        current_cells.add(current_cell)
        if len(current_cells) > 0:
            self.knowledge.append(Sentence(current_cells, current_count))
            ...
            for sentence in self.knowledge:
                for other_sentence in self.knowledge:
                    ...
                    if sentence.cells.issubset(other_sentence.cells):
                        diff_cells = other_sentence.cells - sentence.cells
                        diff_count = other_sentence.count - sentence.count
                        new_sentece = Sentence(diff_cells, diff_count)
                        if new_sentece.cells and new_sentece not in self.knowledge:
                            new_knowledges.append(new_sentece)
                            update = True
